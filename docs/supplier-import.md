...
# Supplier File Intake (Sprint 4)

MVP-загрузка файла поставщика в админке.

## Назначение

- Загрузить файл (CSV) от поставщика
- Сопоставить колонки с полями системы
- Валидировать данные
- Показать результат и статистику
...
## UI: Вкладка "Загрузка файла"

### Шаг 1: Upload
- Поле для названия поставщика
- Drag & drop зона для файла
- Поддержка CSV (мок)

### Шаг 2: Column Mapping
- Выпадающие списки для каждого поля
- Предпросмотр первых 3 строк
- Возврат к предыдущему шагу

### Шаг 3: Preview & Validation
- Статистика: всего, валидных, ошибки, warnings, review candidates
- Таблица результатов
- Детали ошибок и предупреждений

## Validation Rules

### Errors (строка невалидна)
- Пустое название
- Нет цены
- Отрицательная цена
- Нет SKU
...
- `src/lib/types.ts` — типы CsvRow, ColumnMapping, ValidationResult, RawImportRow
- `src/lib/data.ts` — моки (mockCsvData, mockCsvColumns), функции (validateRow, applyMapping, getImportStats)
- `src/app/admin/moderation/page.tsx` — FileIntakeSection компонент

## Следующие шаги

1. Добавить реальную загрузку файлов (File API)
2. Поддержка XLSX
3. Интеграция с SupplierImportItem
4. Сохранение маппинга для каждого поставщика
5. История загрузок
